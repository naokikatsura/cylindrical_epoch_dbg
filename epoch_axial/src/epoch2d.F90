...
PROGRAM pic
  ...
  CALL after_control   ! setup.f90, ('set_initial_values')

!==============================================================================
! 2024/08/07 by katsura
!------------------------------------------------------------------------------
!  CALL output_fields_user_define(step, 0)
!==============================================================================
  ...
  IF (ic_from_restart) THEN
    CALL restart_data(step)   ! setup.f90
  ELSE
    ! auto_load particles
  ...
  CALL manual_load ! ic_module.f90
  ...
  IF (ic_from_restart) THEN
    ...
    IF (step == 0) THEN
      CALL bfield_final_bcs ! boudary.F90
    ELSE
...
! 2024/08/07 by katsura
!------------------------------------------------------------------------------
  CALL output_fields_user_define(step, 1)
!==============================================================================  
...
!      CALL output_fields_user_define(step, 2)
!    END IF
!==============================================================================
