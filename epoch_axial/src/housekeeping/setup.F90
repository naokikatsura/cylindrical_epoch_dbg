...
CONTAINS
  ...
  SUBROUTINE setup_field_boundaries
    ...
    ALLOCATE(exm_x_min(1-ng:ny+ng,0:n_mode-1), exm_x_max(1-ng:ny+ng,0:n_mode-1))
    ALLOCATE(erm_x_min(1-ng:ny+ng,0:n_mode-1), erm_x_max(1-ng:ny+ng,0:n_mode-1))
    ALLOCATE(etm_x_min(1-ng:ny+ng,0:n_mode-1), etm_x_max(1-ng:ny+ng,0:n_mode-1))
    ALLOCATE(bxm_x_min(1-ng:ny+ng,0:n_mode-1), bxm_x_max(1-ng:ny+ng,0:n_mode-1))
    ALLOCATE(brm_x_min(1-ng:ny+ng,0:n_mode-1), brm_x_max(1-ng:ny+ng,0:n_mode-1))
    ALLOCATE(btm_x_min(1-ng:ny+ng,0:n_mode-1), btm_x_max(1-ng:ny+ng,0:n_mode-1))
    ...
    exm_x_min = 0.5_num * (exm(nx0,:,:) + exm(nx0-1,:,:))
    erm_x_min = erm(nx0-1,:,:)
    etm_x_min = etm(nx0-1,:,:)
    exm_x_max = 0.5_num * (exm(nx1,:,:) + exm(nx1+1,:,:))
    erm_x_max = erm(nx1,:,:)
    etm_x_max = etm(nx1,:,:)

    bxm_x_min = bxm(nx0-1,:,:)
    brm_x_min = 0.5_num * (brm(nx0,:,:) + brm(nx0-1,:,:))
    btm_x_min = 0.5_num * (btm(nx0,:,:) + btm(nx0-1,:,:))
    bxm_x_max = bxm(nx1,:,:)
    brm_x_max = 0.5_num * (brm(nx1,:,:) + brm(nx1+1,:,:))
    btm_x_max = 0.5_num * (btm(nx1,:,:) + btm(nx1+1,:,:))
...
! initializing bxm, brm, btm, bxm_old, brm_old, btm_old, exm, erm, etm
!------------------------------------------------------------------------------
    CALL initialise_b_field
    CALL initialise_e_field
!==============================================================================
  ...
  SUBROUTINE initialise_b_field   ! 2024/08/09 by katsura
    ...
    I_coil =  12940.0_num   !!! coil current [A]
    x_coil = -6.0e-4_num    !!! x-coordinate of coil 1 and 2
    y_coil =  6.0e-4_num    !!! y-coordinate of coil 1, -y_coil is that of coil 2
    ! Loop for initialising bxm, brm, btm
    Do im = 0, n_mode-1     !!! Loop over field modes, m
      DO ir = ir_min, ny    !!! Loop over radial cells
        ...
        r_p = r_d + 0.5_num * dy
        im_fac_x = imagi * im / r_d   ! imagi=i, im=m
        im_fac_r = imagi * im / r_p

        !!! r_coil_1 is a distance between coil 1 and the cell
        !!! r_coil_2 is a distance between coil 2 and the cell
        !r_coil_1 = (x(ix) - x_coil) * (x(ix) - x_coil) &
        !         + (y(ir) - y_coil) * (y(ir) - y_coil)
        !r_coil_2 = (x(ix) - x_coil) * (x(ix) - x_coil) &
        !         + (y(ir) + y_coil) * (y(ir) + y_coil)
        !bxm(ix, ir, 0) = mu0 * I_coil / (2.0_num * pi) &
        !    * (((-(y(ir) - y_coil)) / r_coil_1) &
        !    -  ((-(y(ir) + y_coil)) / r_coil_2))
        !brm(ix, ir, 0) = mu0 * I_coil / (2.0_num * pi) &
        !    * ((( (x(ix) - x_coil)) / r_coil_1) &
        !    -  (( (x(ix) - x_coil)) / r_coil_2))
        !bxm_old(ix, ir, 0) = mu0 * I_coil / (2.0_num * pi) &
        !    * (((-(y(ir) - y_coil)) / r_coil_1) &
        !    -  ((-(y(ir) + y_coil)) / r_coil_2))
        !brm_old(ix, ir, 0) = mu0 * I_coil / (2.0_num * pi) &
        !    * ((( (x(ix) - x_coil)) / r_coil_1) &
        !    -  (( (x(ix) - x_coil)) / r_coil_2))
        IF (im == 0) THEN
          bxm(ix,ir,im) = 1.0_num
          brm(ix,ir,im) = 1.0_num
    ...
    IF (y_min_boundary) THEN
      ...
      IF (n_mode > 1) THEN 
        ...
        ! dBy/dt = 0 for initial value - by katsura
        brm(2-ng:nx+ng,0,1) = 0.0_num
    ...
    ! initialize bxm_old, brm_old, btm_old as well
    bxm_old(:,:,:) = bxm(:,:,:)
    brm_old(:,:,:) = brm(:,:,:)
    btm_old(:,:,:) = btm(:,:,:)
  END SUBROUTINE initialise_b_field

  SUBROUTINE initialise_e_field   ! 2024/08/09 by katsura
    ...
    ir_min = 0
    IF (y_min_boundary) ir_min = 1
    ! Loop for initialising exm, erm, etm
    Do im = 0, n_mode-1     !!! Loop over field modes, m
      DO ir = ir_min, ny    !!! Loop over radial cells
        DO ix = 0, nx       !!! Loop over x cells, along cylindrical axis
        ...
        exm(ix,ir,im) = 0.0_num
        erm(ix,ir,im) = 0.0_num
        etm(ix,ir,im) = 0.0_num
    ...
    IF (y_min_boundary) THEN
      ...
      ! dEx/dt = 0 for initial value - by katsura
      exm(1-ng:nx+ng, 0, 0) = 0.0_num
      ...
      END IF
    END IF
  END SUBROUTINE initialise_e_field

  SUBROUTINE initialise_J_field   ! 2024/08/09 by katsura
    INTEGER :: ix, ir, im, ir_min, ir_max
    REAL(num) :: I_coil, x_coil, y_coil, r_coil_1, r_coil_2
    REAL(num) :: r_p, r_d, fac_x, mode_sign
    COMPLEX(num) :: im_fac_x, im_fac_r
    ...
    DO im = 0, n_mode-1   ! Loop over field modes, m
      ...
      END DO
    END DO
  END SUBROUTINE initialise_J_field
  ...
  SUBROUTINE set_dt
    ...
    ! This condition was determined through testing a Gaussian beam in a vacuum,
    ! for cells of dx=dy, dx=2*dy and dy=2*dx. This timestep produced no errors
    dt = 0.9_num * MIN(dx, dy) / SQRT(2.0_num) / c
